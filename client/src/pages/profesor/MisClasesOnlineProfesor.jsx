  ...
  const actualizarLinkZoom = async (claseOnlineId, linkZoom) => {
    try {
      ...
      if (data.success) {
        ...
        await cargarClasesOnline(); // Recargar para mostrar el nuevo link
      } else {
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{
            ...
            Gestiona tus clases online y enlaces de Zoom
          </p>
        </div>
      </div>
      ...
      <Card title={`Clases online - ${selectedDay}`} icon={Clock}>
        {loading ? (
          ...
          <div style={{
            ...
            No hay clases online para este día
          </div>
        ) : (
          <div style={{
            display: 'grid',
...
export default MisClasesOnlineProfesor;
