...
const ProfesoresView = ({ profesores, setProfesores, sedesDisponibles }) => {
  ...
  const profesoresFiltrados = profesores.filter((prof) => {
    const matchSearch =
      (prof.rut &&
        prof.rut.toLowerCase().includes(searchTermProfes.toLowerCase())) ||
      (prof.nombre &&
        prof.nombre.toLowerCase().includes(searchTermProfes.toLowerCase()));
    const matchSede =
      filterSedeProfes === "" ||
      (prof.sedes && prof.sedes.some((s) => s.id === Number(filterSedeProfes)));
    return matchSearch && matchSede;
  });

  // --- FUNCIONES DE TABLA (Originales y Restauradas) ---
  const handleAsignarSedesMasivaProfesores = async () => {
    if (profesoresSeleccionados.length === 0 || sedesMasivasProfes.length === 0)
      return alert("Selecciona profesores y al menos una sede.");
    try {
      const response = await fetch(
        "http://localhost:5000/api/profesores/sedes-profesor",
        {
          method: "PUT",
          headers: {
          ...
          body: JSON.stringify({
            ...
            sedes_ids: sedesMasivasProfes.map(Number),
          }),
        },
      );
      ...
      if (response.ok) {
        ...
        setSedesMasivasProfes([]);
        const profesRes = await fetch("http://localhost:5000/api/profesores", {
  ...
  const handleEliminarProfesores = async (idsArray) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/profesores/eliminar",
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleResetPasswordProfesor = async (idProfesor) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/profesores/reset-password/${idProfesor}`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${authService.getToken()}` },
        },
  ...
  const handleSubmitNuevo = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/profesores/registro",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok && data.success) {
        ...
        setMessage({
          type: "error",
          text: data.message || "Error al registrar el profesor.",
        });
      }
    } catch (error) {
  ...
  const handleEditarProfesor = async (e) => {
    ...
    try {
      const payload = {
        ...
        tipo_contrato: formDataEditar.tipo_contrato,
      };
      const response = await fetch(
        `http://localhost:5000/api/profesores/${profesorEditando.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
        ...
        <Card title="Directorio de Profesores Registrados" icon={Users}>
          ...
          >
            <input
              ...
              value={searchTermProfes}
              onChange={(e) => setSearchTermProfes(e.target.value)}
              style={{
                flex: 2,
                minWidth: "200px",
                padding: "10px",
            ...
            />
            <select
              value={filterSedeProfes}
              onChange={(e) => setFilterSedeProfes(e.target.value)}
              style={{
                flex: 1,
                minWidth: "150px",
                padding: "10px",
            ...
            {profesoresSeleccionados.length > 0 && (
              ...
              >
                ...
                >
                  Asignar Sedes (Ctrl+Click):
                </span>
                <select
                  multiple
                  value={sedesMasivasProfes}
                  onChange={(e) =>
                    setSedesMasivasProfes(
                      Array.from(
                ...
                <Button
                  style={{
                    backgroundColor: colors.secretaria,
                    padding: "8px 16px",
                  }}
                  onClick={handleAsignarSedesMasivaProfesores}
                >
                  Confirmar ({profesoresSeleccionados.length})
                </Button>
                <Button
                  style={{
                  ...
                  onClick={() =>
                    handleEliminarProfesores(profesoresSeleccionados)
                  }
                >
                  <Trash2
                    size={16}
            ...
            >
              <thead>
                ...
                >
                  ...
                  <th style={{ padding: "12px" }}>Sedes Asignadas</th>
                ...
                {profesoresFiltrados.map((prof) => (
                  ...
                  >
                    <td style={{ padding: "12px" }}>
                      <input
                        type="checkbox"
                        checked={profesoresSeleccionados.includes(prof.id)}
                        onChange={(e) =>
                          e.target.checked
                            ? setProfesoresSeleccionados([
                            ...
                            : setProfesoresSeleccionados(
                                profesoresSeleccionados.filter(
                                  (id) => id !== prof.id,
                                ),
                              )
                        }
                    ...
                    <td style={{ padding: "12px" }}>
                      {prof.sedes?.length > 0 ? (
                        <span style={{ color: colors.success }}>
                          {prof.sedes.map((s) => s.nombre).join(", ")}
                        </span>
                      ) : (
                    ...
                    <td style={{ padding: "12px" }}>
                      <button
                        onClick={() => {
                          ...
                          setFormDataEditar({
                            ...
                            tipo_contrato: prof.tipo_contrato || "full_time",
                          });
                      ...
                      <button
                        onClick={() => handleEliminarProfesores([prof.id])}
                        style={{
                          cursor: "pointer",
                          border: "none",
                          background: "none",
          ...
          {message.text && (
            <div
              style={{
                ...
                color:
                  message.type === "error" ? colors.danger : colors.success,
                display: "flex",
                alignItems: "center",
                gap: spacing.gap.normal,
                fontSize: "14px",
          ...
          >
            ...
            >
              ...
              <input
                type="email"
                name="email"
                value={formDataNuevo.email}
                onChange={handleChangeNuevo}
                required
                placeholder="Ej: profesor@escuela.cl"
                style={{
                  padding: "10px",
                  borderRadius: "6px",
                  border: `1px solid ${colors.borderLight}`,
            ...
            >
              ...
              <select
                name="tipo_contrato"
                value={formDataNuevo.tipo_contrato}
                onChange={handleChangeNuevo}
                required
                style={{
                  padding: "10px",
            ...
            <div style={{ gridColumn: "span 2", marginTop: "16px" }}>
              ...
              >
                <Save size={20} />
                {loading ? "Guardando..." : "Guardar Nuevo Profesor"}
              </Button>
            </div>
          </form>
        </Card>
        ...
        <Card title="Editar Datos del Profesor" icon={Edit}>
          ...
          >
            ...
            >
              ...
              <select
                value={formDataEditar.tipo_contrato}
                onChange={(e) =>
                  setFormDataEditar({
                    ...formDataEditar,
            ...
            >
              ...
              <Button
                type="button"
                onClick={() => handleResetPasswordProfesor(profesorEditando.id)}
